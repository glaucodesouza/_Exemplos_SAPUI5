...
{    
    ...
    idAsset: Association[1] to ASSET_VERSION on idAsset.assetId = assetId;
...
entity EVENT : managed
{
    ...
    posLat : Decimal(3,3);
    posLong : Decimal(3,3);
    tracking : Association to many TRACKING on tracking.event = $self;
}
...
entity DEVICE_TYPE : managed
{
    ...
    fromDt : Timestamp;
    toDt : Timestamp;
    devices : Association to many DEVICES on devices.deviceType = $self;
}
...
entity VW_ASSETVS_ASSET AS SELECT FROM ASSET_VERSION {
...
entity VW_FLUXO_PROCESSO_ASSET  
as select  from ASSET_VERSION	as pai
inner join TRACKING 			as tkr		on tkr.assetVs = pai.vsId
inner join ASSET_VERSION		as filho	on filho.trackingId = tkr.trackingId
